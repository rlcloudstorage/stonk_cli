...
def get_arg_value(ctx: dict) -> str:
    """Return first value from arguments list."""
    if DEBUG:
...
def get_option_value(ctx: dict) -> str:
    """Return second value from arguments list."""
    if DEBUG:
        logger.debug(f"get_arg_value(ctx={ctx}")

    return ctx["interface"]["arguments"]
